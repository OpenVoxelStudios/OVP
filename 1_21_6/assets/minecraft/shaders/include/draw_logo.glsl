...
vec3 encodeIntToRGB(float value) {
    int v = int(round(value));
    v = clamp(v, 0, 16777215); // 256^3 - 1, maximum 24-bit color

    float r = float((v >> 16) & 0xFF) / 255.0;
    float g = float((v >> 8) & 0xFF) / 255.0;
    float b = float(v & 0xFF) / 255.0;
